...
`PipelinedDataOnly`, and `PipelinedDedicated`, where the pipeline only overlaps
batch N+1's *write* with batch N's *fsync* — batch N's waiters are still
notified only after batch N's fsync completes.
